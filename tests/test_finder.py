...
def test_wrong_level(f20, min_level, max_level):
    with pytest.raises(AssertionError): NUTSfinder(f20, min_level=min_level, max_level=max_level)


# does the NUTSfinder return the expected number of regions?
@pytest.mark.parametrize("min_level,max_level", levels_valid)
def test_n_find(finder, f20, min_level, max_level, points_inside):
    nf = finder(f20, buffer_geoms=None, min_level=min_level, max_level=max_level)
